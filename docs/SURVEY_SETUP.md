...
## destination — 조건별 목적지도 참가자마다 랜덤 배정됨

기존에는 조건(인간주도/혼합/AI주도)마다 목적지가 고정이었습니다(인간주도→베트남,
혼합→방콕, AI주도→대만, 모든 참가자 동일) — 조건 **순서**는 참가자마다 랜덤이었지만
목적지는 고정이라 목적지 효과가 조건 효과와 뒤섞일 수 있었습니다. 이제
`src/data/conditions.ts`의 `shuffleConditionDestinationMap()`이 조건 순서 랜덤화와는
별개로 조건↔목적지 매핑도 참가자마다 새로 섞어서, 예를 들어 어떤 참가자는
"혼합주도–방콕/인간주도–타이베이/AI주도–다낭", 다른 참가자는
"AI주도–타이베이/혼합주도–다낭/인간주도–방콕" 처럼 조합이 달라집니다. 링크는 여전히
하나입니다 — 참가자가 동의 화면을 통과하는 시점(`acceptConsent`)에 매핑이 한 번 정해져
그대로 유지됩니다.

이 조합을 실제로 기록하기 위해 `destination` 필드를 폼에 추가해 entry
ID(`entry.227310911`)를 전달받아 `src/lib/surveyFormFields.ts`에 반영했습니다 — 조건
행(사전 설문/최종 행 제외)마다 그 행에서 실제로 사용된 목적지명이 채워집니다. 위 표의
목적지 값(방콕/베트남/대만)은 예시일 뿐이며, 실제로는 참가자마다 어느 조건에 어느
목적지가 붙는지 달라집니다.
